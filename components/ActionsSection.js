...
export default function ActionsSection({ action_types_and_varieties_data }) {
  ...
  types_and_vars_sorted.map(([type, varieties]) => ({type, count: objSum(varieties)})),[types_and_vars_sorted])
  ...
  return (
    <SectionContainer>
      ...
      <br />
      <BarChart width={730} height={350} data={actor_types_cumulative.filter((item) =>!['environmental', 'unknown'].includes(item.type))} layout={'horizontal'} >
  ...
  <Bar dataKey="count" fill="#8884d8"  />
      ...
      {types_and_vars_sorted.filter(item=>item[0] !== 'unknown').map(([type, varieties]) => (
        <div className='mt-6' key={type}>
          <h3 className="text-2xl">{`${objSum(varieties)} incidents were due to ${
            type
          }`}</h3>
          <TableContainer className='mt-2'>
            <Table variant="simple" size="sm">
              ...
              <Tbody>
                {Object.entries(varieties).sort((a,b)=> b[1] - a[1]).map(([variety, count]) => {
